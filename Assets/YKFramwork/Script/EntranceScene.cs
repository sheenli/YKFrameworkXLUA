...
public class EntranceScene : SceneBase
{
    ...
    {
        base.OnEnter();
        this.AddListener(SceneMgr.Instance, AppConst.CoreDef.INITHOTUPDATA);
        this.AddListener(SceneMgr.Instance, AppConst.CoreDef.INSTALLATIONED);
        this.AddListener(SceneMgr.Instance, AppConst.CoreDef.DOWNED);
        this.AddListener(SceneMgr.Instance, AppConst.CoreDef.LoadLuaFinished);
    ...
    private void DownLoadFile()
    {
        Action<HotUpdateRessMgr.DecompressionOrDownInfo> checkFileed = downloadInfo =>
        {
            ...
            {
                if (downloadInfo.comparisonInfo.flag == 2)
                {
                    DeleteAllKey(true);
                }
                AsynTask task = new AsynTask(true, () =>
